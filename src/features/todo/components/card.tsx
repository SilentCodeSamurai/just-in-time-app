...
export function TodoCard({ todo }: { todo: TodoAllItem }) {
	...
	const diffHours = todo.dueDate
		? differenceInHours(getDueTime(todo.dueDate), now.current)
		: Infinity;
	...
	const handleChangeStatus = async (completed: boolean) => {
		setIsUpdating(true);
		try {
			await db.todos.where({ id: todo.id }).modify({
				completed,
	...
	return (
		<>
			...
			<motion.div animate={controls}>
				<Card
					variant="item"
					className={`relative w-full gap-1 pl-2 lg:pl-3`}
				>
					{todo.completed && (
						<div className="top-0 right-0 bottom-0 left-0 z-10 absolute bg-neutral-300 dark:bg-neutral-900 opacity-60 rounded-lg pointer-events-none" />
					)}
					...
					<CardHeader>
						...
						<CardTitle
							className={`text-sm lg:text-xl ${todo.completed ? "line-through opacity-50" : ""}`}
						>
							{todo.title}
						</CardTitle>
					</CardHeader>
					<CardContent>
						<p
							className={`text-sm ${todo.completed ? "line-through opacity-50" : ""}`}
						>
							{todo.description}
						</p>
					</CardContent>
